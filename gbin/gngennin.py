...
def main():
    ...
    parser.add_argument(
        '-j', help="ninja's -j, the number of processes", type=int)
    parser.add_argument(
        '-f', help="ignores build errors and create sln", action='store_true')
    ...
    if ret != 0 and not args.f:
        myexit(ret=ret)

    # gn gen
    tmpSlnFileName = targetName + '_gngennin'
    gngenarg = ['gn', 'gen', '--ide=vs2022', '--filters=' +
                fullTarget, '--sln=' + tmpSlnFileName, outdir]
    print(gngenarg)
    ...
    slnStartupProjectExe = os.path.join(
        THISDIR, "slnStartupProject\\bin\\slnStartupProject.exe")
    if not os.path.isfile(slnStartupProjectExe):
        myexit(
            r"""'{}' is not found. Run 'git submodule update -i'""".format(slnStartupProjectExe))
